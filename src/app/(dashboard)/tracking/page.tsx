  ...
  return (
    <div className="p-6 sm:p-8 h-full flex flex-col gap-6 overflow-y-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-semibold tracking-tight text-neutral-900">Tracking</h1>
          <p className="text-sm text-neutral-500 mt-1">Real-time visibility into your shipments.</p>
        </div>
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-3 h-11 px-4 rounded-xl bg-white border border-neutral-200 shadow-sm hover:border-orange-300 hover:shadow-orange-100/60 hover:shadow-md transition-all duration-200 text-sm font-medium text-neutral-800 min-w-[220px]"
          >
            <div className="flex items-center gap-2 flex-1 min-w-0">
              ...
              <span className="text-neutral-500 truncate text-xs">{shipment.destination}</span>
            </div>
                    ...
                    return (
                      <li key={s.shipment_id}>
                        <button
                          ...
                          className={`w-full flex items-center gap-3 px-3 py-2.5 text-left transition-colors ${isActive ? "bg-orange-50" : "hover:bg-neutral-50"}`}
                        >
                          <div className={`w-2 h-2 rounded-full shrink-0 ${ssc.dot}`} />
                          <div className="flex-1 min-w-0">
                            <p className={`text-sm font-medium ${isActive ? "text-orange-700" : "text-neutral-800"}`}>{s.shipment_id}</p>
                            <p className="text-xs text-neutral-500 truncate">{s.origin} → {s.destination}</p>
                          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-5 flex-1 min-h-0">
        ...
        <div className="flex flex-col gap-5 min-h-0">
          ...
          <div className="bg-white rounded-2xl border border-neutral-200/70 shadow-sm p-6">
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div>
                <p className="text-xs font-semibold text-neutral-400 uppercase tracking-wider mb-1">Shipment ID</p>
                <h2 className="text-2xl font-bold text-neutral-900 tracking-tight">{shipment.shipment_id}</h2>
              </div>
            ...
            <div className="mt-5">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-medium text-neutral-500">Delivery Progress</span>
          ...
          {/* Route Card */}
          <div className="bg-white rounded-2xl border border-neutral-200/70 shadow-sm p-6 flex-1">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-sm font-semibold text-neutral-900 uppercase tracking-wider">Route & History</h3>
              <span className="text-xs text-neutral-400">{completedEvents.length} of {totalSteps} checkpoints</span>
            </div>

            {/* Origin → Destination banner */}
            <div className="flex items-center gap-3 p-3.5 rounded-xl bg-gradient-to-r from-orange-50 to-amber-50 border border-orange-100 mb-6">
              <div className="flex items-center gap-2 flex-1 min-w-0">
                <div className="w-2.5 h-2.5 rounded-full bg-neutral-800 shrink-0" />
                <span className="text-sm font-semibold text-neutral-800 truncate">{shipment.origin}</span>
              </div>
              <ArrowRight className="w-4 h-4 text-orange-400 shrink-0" />
              <div className="flex items-center gap-2 flex-1 min-w-0 justify-end">
                <span className="text-sm font-semibold text-neutral-800 truncate text-right">{shipment.destination}</span>
              ...
              <div className="space-y-1">
                {shipment.events.map((event, idx) => (
                  <div key={idx} className="relative flex gap-4 pb-5 last:pb-0">
                    {/* Icon */}
                    <div className={`relative z-10 w-8 h-8 rounded-full shrink-0 flex items-center justify-center border-2 border-white shadow-sm ${
                      event.completed ? "bg-orange-500" : "bg-neutral-100 border-neutral-200"
                    }`}>
                      {event.completed
                    ...
                    <div className="flex-1 pt-0.5 min-w-0">
                      <div className="flex items-baseline justify-between gap-2 flex-wrap">
                        <span className={`text-sm font-semibold ${event.completed ? "text-neutral-900" : "text-neutral-400"}`}>
                          {event.title}
                        </span>
                        <span className={`text-xs font-medium shrink-0 ${event.completed ? "text-neutral-500" : "text-neutral-400"}`}>
                          {event.date}
                        </span>
                      </div>
                      <p className={`text-xs mt-0.5 leading-relaxed ${event.completed ? "text-neutral-500" : "text-neutral-400"}`}>
                        {event.description}
                      </p>
                    </div>
                  </div>
        ...
        <div className="flex flex-col gap-5 min-h-0">
          ...
          {/* Map Card */}
          <div className="bg-white rounded-2xl border border-neutral-200/70 shadow-sm overflow-hidden flex-1 flex flex-col min-h-[320px]">
            <div className="flex items-center justify-between px-5 py-4 border-b border-neutral-100">
              <h3 className="text-sm font-semibold text-neutral-900">Route Map</h3>
            ...
            {/* Map + overlay */}
            <div className="relative flex-1 bg-neutral-100 min-h-[240px]">
              <iframe
                ...
                style={{ border: 0, mixBlendMode: "multiply" }}
            ...
            {/* Map footer */}
            <div className="px-5 py-3 border-t border-neutral-100 flex items-center justify-between bg-neutral-50/60">
              ...
              <span className="text-[11px] text-neutral-400 font-medium">Sri Lanka</span>
            </div>
          </div>

          {/* Snapshot Stats Card */}
          <div className="bg-white rounded-2xl border border-neutral-200/70 shadow-sm p-5">
            <h3 className="text-xs font-semibold text-neutral-400 uppercase tracking-wider mb-4">Shipment Snapshot</h3>
            <div className="grid grid-cols-2 gap-3">
              {[
                { label: "Status",        value: sc.label,                       color: sc.text },
                { label: "Checkpoints",   value: `${completedEvents.length}/${totalSteps}`, color: "text-neutral-800" },
                { label: "Invoice",       value: shipment.invoice_id,            color: "text-neutral-800" },
                { label: "Est. Delivery", value: formatDate(shipment.estimated_delivery), color: "text-neutral-800" },
              ].map(({ label, value, color }) => (
                <div key={label} className="rounded-xl bg-neutral-50 border border-neutral-100 p-3">
